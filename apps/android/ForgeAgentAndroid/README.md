# DeepSeek-Forge Android

DeepSeek-Forge Android is a paired remote WebView device for DeepSeek-Forge running on
...
1. Start DeepSeek-Forge on the Mac with the macOS app or local Web Console.
2. Open **Pair Mobile** on the Mac.
3. Open the Android app and tap **Scan Pair Mobile QR**.
4. After pairing, the app opens the full DeepSeek-Forge Web Console.
...
the phone can still reach DeepSeek-Forge. Tapping the notification returns to the
Web Console. The service restarts after device boot or app update if the device
is still paired.

The same service also listens to DeepSeek-Forge's authenticated event stream and
...
This background service does not run a DeepSeek-Forge Core on Android. The Core
